...
import { Briefcase, Plus, Users, Clock, ArrowRight, Folder, PlusCircle, X, Globe, LogOut } from 'lucide-react';
...
const Home = () => {
  ...
  const handleCreateWorkspace = async (e) => {
    ...
    try {
      setCreating(true);
      const result = await workspaceApi.create({ name: newWorkspaceName, projectId: PROJECT_ID });
      if (result.success) {
  ...
  const handleEnter = (ws) => {
    setWorkspace(ws.workspace_id, ws.name);
    navigate('/dashboard');
  };
    ...
    <div className="min-h-screen bg-white text-black flex flex-col font-sans">
      ...
      <main className="flex-1 w-full px-8 md:px-24 py-24 flex flex-col gap-20 max-w-[1800px] mx-auto">
        ...
        <div className="text-center md:text-left">
          ...
          {error && <p className="text-xs text-red-500 mt-6 font-black uppercase tracking-widest flex items-center gap-2"><div className="w-2 h-2 bg-red-500"></div> {error}</p>}
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="bg-white border-[6px] border-black max-w-md w-full p-12 flex flex-col gap-10 shadow-[0_60px_120px_rgba(0,0,0,0.6)]">
            ...
            <form onSubmit={handleCreateWorkspace} className="space-y-10">
              <div className="flex flex-col gap-4">
                <label className="text-[11px] font-black text-black/80 uppercase tracking-[0.4em]">Workspace Designation</label>
                <input
                  type="text"
                  required
                  ...
                  className="border-4 border-black/20 focus:border-black p-5 text-base font-black uppercase tracking-widest focus:outline-none w-full text-black placeholder-black/40"
                />
              </div>
              <div className="flex flex-col gap-4">
                <label className="text-[11px] font-black text-black/80 uppercase tracking-[0.4em]">Mission Parameters</label>
                <textarea
                  rows={3}
                  value={newWorkspaceDesc}
                  onChange={(e) => setNewWorkspaceDesc(e.target.value)}
                  placeholder="DESCRIPTION"
                  className="border-4 border-black/20 focus:border-black p-5 text-base font-black uppercase tracking-widest focus:outline-none w-full text-black placeholder-black/40 leading-relaxed resize-none"
                />
              </div>
              <div className="flex justify-end items-center gap-8 pt-10 border-t-4 border-black/5 mt-6">
                <button type="button" onClick={() => setIsModalOpen(false)} className="text-[11px] font-black uppercase tracking-[0.3em] text-black/60 hover:text-black transition-colors">
                  Abort
                </button>
                <button type="submit" disabled={creating} className="px-10 py-5 bg-black text-white text-[11px] font-black uppercase tracking-[0.4em] hover:invert transition-all disabled:opacity-50">
